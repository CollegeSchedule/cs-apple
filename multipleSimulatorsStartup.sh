...
path=$(find ~/Library/Developer/Xcode/DerivedData/cs-apple-*/Build/Products/Debug-iphonesimulator -name "cs-apple.app" | head -n 1)
echo "${path}"
...
xcrun simctl launch "$line" me.whywelive.cs-apple # Launch .app using its bundle at simulator with identifier hold by $line var
done
